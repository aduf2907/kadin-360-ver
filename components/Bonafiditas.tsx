...
const statusConfig = {
    Green: {
        ...
        description: 'Your account is fully trusted and has access to all platform features.',
        privileges: [
    ...
    Yellow: {
        ...
        description: 'Your account requires additional information or activity to be fully verified.',
        privileges: [
    ...
    Red: {
        ...
        description: 'Your account is under observation due to data anomalies or community reports.',
        privileges: [
    ...
    Black: {
        ...
        description: 'This account has been suspended due to severe violations or failed verification.',
        privileges: [
...
const Bonafiditas: React.FC<BonafiditasProps> = ({ user }) => {
    ...
    const status = statusConfig[user.bonafidityStatus];
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold text-kadin-white mb-2">User Bonafidity</h2>
            <p className="text-kadin-light-slate mb-8">
                Understand your trust and reputation score within the KADIN 360 ecosystem.
            </p>

            {/* Current Status Card */}
            <div className={`p-6 rounded-lg border ${status.borderColor} ${status.bgColor} mb-12`}>
                <div className="flex flex-col md:flex-row items-center gap-6">
                    <div className="text-6xl">{status.emoji}</div>
                    <div className="flex-1 text-center md:text-left">
                        <p className="text-sm font-bold uppercase tracking-wider text-kadin-slate">Your Current Status</p>
                        <h3 className={`text-2xl font-bold ${status.color}`}>{status.title}</h3>
                        <p className="text-kadin-slate text-sm mt-1">{status.description}</p>
                    </div>
                    <div className="flex-shrink-0 text-center">
                        <p className="text-sm font-bold uppercase tracking-wider text-kadin-slate">Bonafidity Score</p>
                        <p className={`text-4xl font-extrabold ${status.color}`}>{user.rating}<span className="text-2xl text-kadin-slate">/100</span></p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-kadin-light-navy p-6 rounded-lg border border-gray-700">
                    <h4 className="text-xl font-bold text-kadin-white mb-4">Your Access & Privileges</h4>
                    <ul className="space-y-3">
                        {status.privileges.map((item, index) => (
                            <li key={index} className="flex items-start">
                                <svg className="w-5 h-5 mr-3 mt-0.5 flex-shrink-0 text-green-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {status.improvements && status.improvements.length > 0 && (
                     <div className="bg-kadin-light-navy p-6 rounded-lg border border-gray-700">
                        <h4 className="text-xl font-bold text-kadin-white mb-4">How to Improve Your Status</h4>
                        <ul className="space-y-3">
                            {status.improvements.map((item, index) => (
                                <li key={index} className="flex items-start">
                                    <svg className="w-5 h-5 mr-3 mt-0.5 flex-shrink-0 text-kadin-gold" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="mt-12">
                <h3 className="text-2xl font-bold text-kadin-white text-center mb-6">Bonafidity Levels Explained</h3>
                <div className="space-y-2">
                    {bonafidityLevels.map((level, index) => (
                        <div key={index} className={`border rounded-lg ${openLevel === index ? level.borderColor : 'border-gray-700'} ${level.bgColor}`}>
                             <button
                                className="w-full flex justify-between items-center text-left p-4"
                                onClick={() => toggleLevel(index)}
                            >
                                <span className={`font-bold text-lg ${level.color}`}>{level.emoji} {level.title}</span>
                                <ChevronDownIcon className={`w-5 h-5 text-kadin-slate transform transition-transform duration-300 ${openLevel === index ? 'rotate-180' : ''}`} />
                            </button>
                            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openLevel === index ? 'max-h-96' : 'max-h-0'}`}>
                                <div className="p-4 pt-0">
                                    <div className="border-t border-gray-700/50 pt-4">
                                        <h5 className="font-bold text-kadin-white mb-2">Criteria:</h5>
                                        <ul className="space-y-1 list-disc list-inside text-sm text-kadin-slate">
                                            {level.criteria.map((item, i) => <li key={i}>{item}</li>)}
                                        </ul>
                                        <h5 className="font-bold text-kadin-white mt-4 mb-2">Access Rights:</h5>
                                        <p className="text-sm text-kadin-slate">{level.privileges.join(', ')}.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
