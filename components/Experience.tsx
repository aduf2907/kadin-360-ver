...
import { ExperienceEntry, UserProfile } from "../types";
...
const Experience: React.FC<{ user: UserProfile }> = ({ user }) => {
  const { entries, loading, addEntry } = useExperiences(true);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("experiences").getPublicUrl(filePath);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      alert(
        "Experience shared successfully! It will be visible after admin approval.",
      );
    } else {
      alert("Failed to share experience: " + result.error);
    }
    setIsSaving(false);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-kadin-slate mt-1">
            Share and learn from the journeys of fellow business leaders.
          </p>
        </div>
        <button
              ...
              <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar">
                ...
                <div>
                  ...
                  <textarea
                    id="content"
                    required
                    placeholder="Share your journey, lessons learned, or success story..."
