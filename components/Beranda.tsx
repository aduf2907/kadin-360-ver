...
const FeatureCard: React.FC<{
    ...
    >
        ...
        <h3 className="text-lg font-bold text-kadin-white mb-2 transition-colors duration-200 group-hover:text-kadin-gold">{title}</h3>
        <p className="text-kadin-slate text-xs leading-relaxed flex-grow">{description}</p>
    </div>
);
...
const heroContent = [
    {
        ...
        title: <>Selamat Datang di <br/><span className="text-kadin-gold">KADIN 360</span></>,
        subtitle: "Platform terintegrasi untuk komunikasi, berbagi pengetahuan, dan business matching eksklusif bagi anggota KADIN Indonesia."
    },
    {
        tag: "AI POWERED MATCHING",
        title: <>Temukan <span className="text-kadin-gold">Peluang Bisnis</span> <br/>Tanpa Batas</>,
        subtitle: "Akses fitur Business Matching dengan rekomendasi AI untuk menemukan mitra yang paling sesuai dengan profil bisnis Anda."
    },
    {
        tag: "NETWORKING",
        title: <>Perluas Jaringan & <br/><span className="text-kadin-gold">Kolaborasi Global</span></>,
        subtitle: "Terhubung dengan para pemimpin industri dan sesama anggota melalui forum diskusi, komunitas, dan acara eksklusif."
    },
     {
        tag: "KNOWLEDGE HUB",
        title: <>Tingkatkan Wawasan, <br/><span className="text-kadin-gold">Kembangkan Bisnis</span></>,
        subtitle: "Manfaatkan pusat pengetahuan kami yang berisi e-learning, artikel, dan wawasan pasar terkini untuk pertumbuhan bisnis Anda."
    }
];
...
const Beranda: React.FC<BerandaProps> = ({ setCurrentPage }) => {
    ...
    const [heroAnimationState, setHeroAnimationState] = useState<'enter' | 'exit'>('enter');

    useEffect(() => {
        const heroImageInterval = setInterval(() => {
            setHeroAnimationState('exit');
            setTimeout(() => {
                setCurrentHeroImageIndex(prevIndex => (prevIndex + 1) % heroCarouselImages.length);
        ...
        const featureImageInterval = setInterval(() => {
            setCurrentFeatureImageIndex(prevIndex => (prevIndex + 1) % featureCarouselImages.length);
        }, 5000);
        ...
        const handleTyping = () => {
            ...
            if (isDeleting) {
                if (displayedSubtitle === '') {
                    ...
                    setDisplayedSubtitle(fullText.substring(0, displayedSubtitle.length - 1));
                    setTypingSpeed(100);
                }
            } else {
    ...
    const goToPreviousHero = () => {
        ...
        setTimeout(() => {
            ...
            const newIndex = isFirstSlide ? heroCarouselImages.length - 1 : currentHeroImageIndex - 1;
    ...
    const goToNextHero = () => {
        setHeroAnimationState('exit');
        setTimeout(() => {
            const isLastSlide = currentHeroImageIndex === heroCarouselImages.length - 1;
    ...
    const goToPreviousFeature = () => {
        ...
        const newIndex = isFirstSlide ? featureCarouselImages.length - 1 : currentFeatureImageIndex - 1;
        setCurrentFeatureImageIndex(newIndex);
    };
    const goToNextFeature = () => {
        const isLastSlide = currentFeatureImageIndex === featureCarouselImages.length - 1;
    ...
    return (
        <div className="text-center pb-12">
                ...
                <div className="relative z-30 h-full flex flex-col justify-center px-6 md:px-12 lg:px-16">
                    ...
                    >
                        <div className="inline-flex items-center space-x-2 bg-kadin-gold/10 border border-kadin-gold/30 rounded-full px-3 py-1 mb-4 backdrop-blur-sm">
                            ...
                            <span className="text-[10px] font-bold tracking-widest text-kadin-gold uppercase">{heroContent[currentHeroIndex].tag}</span>
                        </div>
            ...
            <div className="mt-12 max-w-7xl mx-auto px-6 text-left">
                ...
                <div className="relative rounded-xl overflow-hidden py-10 mb-6 group shadow-lg">
                    ...
                    <div className="relative z-10 text-center">
                        <h2 className="text-2xl md:text-3xl font-bold text-kadin-white">Fitur Unggulan Kami</h2>
            ...
            <div className="mt-12 max-w-7xl mx-auto px-6">
                 <h2 className="text-2xl font-bold text-kadin-white mb-6 text-left border-l-4 border-kadin-gold pl-4">Aktivitas Terkini</h2>
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-left">
                    {/* KADIN News */}
                    <div className="bg-kadin-light-navy p-6 rounded-lg border border-gray-700 cursor-pointer hover:border-kadin-gold/50 transition-all duration-300 transform hover:-translate-y-0.5 group shadow-lg" onClick={() => setCurrentPage('Knowledge')}>
                        <div className="flex justify-between items-center mb-6">
                             <h3 className="text-lg font-bold text-kadin-white transition-colors duration-200 group-hover:text-kadin-gold">Berita Terkini KADIN</h3>
                             <a onClick={(e) => { e.stopPropagation(); setCurrentPage('Knowledge')}} className="text-xs text-kadin-gold hover:underline cursor-pointer font-bold">
                                 Lihat Semua
                             </a>
                        </div>
                        <div className="space-y-4">
                            {mockNews.map(item => (
                                <div key={item.id} className="flex items-start space-x-4 group/item cursor-pointer" onClick={(e) => { e.stopPropagation(); setCurrentPage('Knowledge'); }}>
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-kadin-white group-hover/item:text-kadin-gold text-xs leading-snug">{item.title}</h4>
                                        <p className="text-[10px] text-kadin-slate mt-1">{item.category} • {item.date}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {/* Forum Discussions */}
                    <div className="bg-kadin-light-navy p-6 rounded-lg border border-gray-700 cursor-pointer hover:border-kadin-gold/50 transition-all duration-300 transform hover:-translate-y-0.5 group shadow-lg" onClick={() => setCurrentPage('KADINers Rooms')}>
                         <div className="flex justify-between items-center mb-6">
                             <h3 className="text-lg font-bold text-kadin-white transition-colors duration-200 group-hover:text-kadin-gold">Diskusi Forum Terkini</h3>
                             <a onClick={(e) => {e.stopPropagation(); setCurrentPage('KADINers Rooms')}} className="text-xs text-kadin-gold hover:underline cursor-pointer font-bold">
                                 Masuk ke Forum
                             </a>
                        </div>
                        <div className="space-y-3">
                            {mockThreads.map(thread => (
                                <div key={thread.id} className="p-4 rounded-lg bg-kadin-navy/40 hover:bg-kadin-navy group/item cursor-pointer border border-transparent hover:border-gray-600 transition-all shadow-inner" onClick={(e) => { e.stopPropagation(); setCurrentPage('KADINers Rooms'); }}>
                                    <h4 className="font-semibold text-kadin-white group-hover/item:text-kadin-gold text-xs">{thread.title}</h4>
                                    <p className="text-[10px] text-kadin-slate mt-1">
                                        Posted in <span className="font-semibold text-kadin-light-slate">{thread.category}</span> by {thread.author}
                                    </p>
                                </div>
                            ))}
                        </div>
