...
const MembershipCard: React.FC<MembershipCardProps> = ({ user }) => {
    ...
    return (
        <div>
            <h2 className="text-3xl font-bold text-kadin-white mb-2">Digital Membership Card</h2>
            <p className="text-kadin-light-slate mb-8">
                Present this digital card for access to exclusive KADIN events and partner benefits.
            </p>
            ...
            <div className="max-w-xl mx-auto" style={{ perspective: '1000px' }}>
                ...
                <div className="w-full bg-gradient-to-br from-kadin-navy to-kadin-light-navy p-6 rounded-2xl shadow-2xl border border-gray-700/50 transition-transform duration-500 ease-in-out hover:rotate-y-3">
                    ...
                    <div className="flex justify-between items-center pb-4 border-b border-kadin-gold/20">
                        ...
                        <span className={`px-3 py-1 text-xs font-bold rounded-full ${levelColor[user.level]}`}>
                            {user.level} Member
                        </span>
                    </div>
                    ...
                    <div className="flex items-center mt-6">
                        <img src={user.avatar} alt={user.name} className="w-28 h-28 rounded-full border-4 border-kadin-gold" />
                        <div className="ml-6 flex-1">
                            <p className="text-xs text-kadin-slate font-semibold">NAME</p>
                            <h4 className="text-2xl font-bold text-kadin-white">{user.name}</h4>

                            <p className="text-xs text-kadin-slate font-semibold mt-2">COMPANY</p>
                    ...
                    <div className="mt-6 flex justify-between items-end">
                        <div>
                            <p className="text-xs text-kadin-slate font-semibold">MEMBER ID</p>
                            <p className="font-mono text-lg text-kadin-white tracking-wider">{user.membershipId}</p>

                            <p className="text-xs text-kadin-slate font-semibold mt-2">VALID THRU</p>
                            <p className="font-mono text-md text-kadin-light-slate">{user.validThru}</p>
                        </div>
                        <img 
