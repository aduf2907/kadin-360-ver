...
import { StoryEntry, UserProfile } from "../types";
...
const StoryCard: React.FC<{ entry: StoryEntry; onClick: () => void }> = ({
  entry,
  onClick,
}) => (
  <div
...
const MyStory: React.FC<{ user: UserProfile }> = ({ user }) => {
  const { entries, loading, addEntry } = useStories(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      alert(
        "Story shared successfully! It will be visible after admin approval.",
      );
    } else {
      alert("Failed to share story: " + result.error);
    }
    setIsSaving(false);
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-kadin-white">My Story</h2>
          <p className="text-kadin-slate mt-1">
            Inspiring narratives from the KADIN community.
          </p>
        </div>
        <button
          ...
          <div className="bg-kadin-light-navy border border-gray-700 rounded-xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
            ...
            >
              <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar">
                ...
                <div>
                  ...
                  <textarea
                    id="content"
                    required
                    placeholder="Share your story, journey, or insights..."
              ...
              <div className="p-6 border-t border-gray-700 flex justify-end gap-3 bg-kadin-light-navy">
                ...
                >
                  {isSaving ? "Sharing..." : "Share for Review"}
                </button>
              </div>
            </form>
          </div>
