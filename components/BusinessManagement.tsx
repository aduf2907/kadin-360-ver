...
import { BusinessConsultation, BusinessAdvisor, UserProfile } from "../types";
...
const BusinessManagement: React.FC<BusinessManagementProps> = ({ user }) => {
  const {
    advisors,
    consultations,
    loading,
  ...
  const [selectedConsultation, setSelectedConsultation] =
    useState<BusinessConsultation | null>(null);
  ...
  const handleRespond = async (status: BusinessConsultation["status"]) => {
              ...
              <Card key={advisor.id} title={advisor.name}>
                <div className="flex items-center gap-4 mb-4">
                  <img
                    src={
                      advisor.avatar_url ||
                      "https://picsum.photos/seed/advisor/100/100"
                    }
                    alt={advisor.name}
