...
// Icons for Dropdown
const ProfileDropdownIcon: React.FC<{ className?: string }> = ({
  className,
}) => (
  <svg
    className={className}
...
const Header: React.FC<HeaderProps> = ({
  ...
  const [notificationPrefs, setNotificationPrefs] = useState(() => {
    try {
      ...
      console.error(
        "Could not parse notification preferences from localStorage",
        error,
      );
      return {
        messages: true,
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        "Could not save notification preferences to localStorage",
        error,
      );
    }
  }, [notificationPrefs]);
    ...
    <header className="flex-shrink-0 bg-kadin-light-navy/80 backdrop-blur-md border-b border-gray-700 h-20 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20">
      <div className="flex items-center">
        <button
          ...
          className="lg:hidden text-kadin-slate mr-4"
        >
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="relative" ref={notificationDropdownRef}>
          <button
            ...
            className="text-kadin-slate hover:text-kadin-white p-2 rounded-full hover:bg-kadin-navy"
          >
            ...
            <div className="absolute right-0 mt-2 w-72 origin-top-right bg-kadin-light-navy rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-20">
              ...
              <div className="py-2 px-4 space-y-3">
                <label className="flex items-center justify-between text-sm">
                  ...
                  <input
                    type="checkbox"
                    className="form-checkbox h-4 w-4 text-kadin-gold bg-kadin-navy border-gray-600 rounded focus:ring-kadin-gold"
        ...
        <div className="relative" ref={profileDropdownRef}>
          ...
          >
            ...
            <div className="hidden md:block text-left">
              <p className="text-sm font-semibold text-kadin-white">
                {loading ? "Loading..." : profile?.name || "Guest User"}
              </p>
              <p className="text-xs text-kadin-slate">
            ...
            <div className="absolute right-0 mt-2 w-56 origin-top-right bg-kadin-light-navy rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-20">
              <div className="py-1">
                <a
                  ...
                  className="flex items-center px-4 py-2 text-sm text-kadin-slate hover:bg-kadin-navy hover:text-kadin-white cursor-pointer"
                >
                ...
                <a
                  onClick={() => {
                    setCurrentPage("Membership Card");
                    setProfileOpen(false);
                  }}
                  className="flex items-center px-4 py-2 text-sm text-kadin-slate hover:bg-kadin-navy hover:text-kadin-white cursor-pointer"
                >
