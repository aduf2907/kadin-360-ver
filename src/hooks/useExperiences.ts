...
export const useExperiences = (onlyApproved: boolean = true) => {
  ...
  const fetchEntries = async () => {
    try {
      ...
      if (data) {
        const mappedData: ExperienceEntry[] = data.map((item: any) => ({
          id: item.id,
          ...
          image_url:
            item.image_url ||
            `https://picsum.photos/seed/experience${item.id}/400/200`,
  ...
  const updateEntryStatus = async (
    ...
    try {
      ...
      console.error("Error updating experience entry status:", err);
      return { success: false, error: err.message };
    }
  };
