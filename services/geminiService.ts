...
const mockUsers: UserProfile[] = [
    { id: 1, name: 'Citra Lestari', company: 'Agro Makmur Sejahtera', role: 'CEO', avatar: 'https://picsum.photos/id/1011/200/200', industry: 'Agriculture', region: 'Surabaya', interests: ['export', 'sustainability'], isAiRecommended: true, level: 'Premium', membershipId: 'KDN-87654321', validThru: '11/26', bonafidityStatus: 'Green', rating: 95, reviews: [
    ...
    { id: 2, name: 'Andi Wijaya', company: 'PT. Maju Logistik', role: 'Operations Manager', avatar: 'https://picsum.photos/id/1027/200/200', industry: 'Technology', region: 'Jakarta', interests: ['supply chain', 'iot'], isAiRecommended: true, level: 'Verified', membershipId: 'KDN-87654322', validThru: '10/25', bonafidityStatus: 'Green', rating: 88, reviews: [] },
    { id: 3, name: 'Rina Hartono', company: 'Karya Manufaktur', role: 'Director', avatar: 'https://picsum.photos/id/1012/200/200', industry: 'Manufacturing', region: 'Bandung', interests: ['automation', 'quality control'], isAiRecommended: false, level: 'Active', membershipId: 'KDN-87654323', validThru: '09/26', bonafidityStatus: 'Yellow', rating: 72, reviews: [] },
    { id: 4, name: 'David Lee', company: 'Innovate Solutions', role: 'CTO', avatar: 'https://picsum.photos/id/1013/200/200', industry: 'Technology', region: 'International', interests: ['saas', 'ai'], isAiRecommended: true, level: 'Premium', membershipId: 'KDN-87654324', validThru: '08/25', bonafidityStatus: 'Green', rating: 98, reviews: [] },
    { id: 5, name: 'Siti Aminah', company: 'Green Energy Corp', role: 'Founder', avatar: 'https://picsum.photos/id/1014/200/200', industry: 'Energy', region: 'Jakarta', interests: ['renewable energy', 'investment'], isAiRecommended: false, level: 'Verified', membershipId: 'KDN-87654325', validThru: '07/26', bonafidityStatus: 'Green', rating: 90, reviews: [] },
    { id: 6, name: 'Bambang Susilo', company: 'Nusantara Foods', role: 'Marketing Head', avatar: 'https://picsum.photos/id/1015/200/200', industry: 'F&B', region: 'Surabaya', interests: ['branding', 'digital marketing'], isAiRecommended: false, level: 'Active', membershipId: 'KDN-87654326', validThru: '06/25', bonafidityStatus: 'Yellow', rating: 68, reviews: [] },
];
...
export const getAiRecommendations = async (): Promise<UserProfile[]> => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        
        // Sending a more concise payload to the model
        const partnerProfiles = mockUsers.map(({ id, industry, region, interests, level }) => 
            ({ id, industry, region, interests, level }));

        const prompt = `
            The current user's profile is: Industry: Technology, Region: Jakarta, Interests: ${currentUser.interests.join(', ')}.
            From the following list of potential partners, please recommend up to 4 that are the best match.
        ...
        const recommendedIdsData = JSON.parse(response.text) as { id: number }[];
        const recommendedIds = new Set(recommendedIdsData.map(u => u.id));
        
        // Return all users, with the 'isAiRecommended' flag set dynamically based on the AI response
        return mockUsers.map(user => ({
            ...user,
            isAiRecommended: recommendedIds.has(user.id),
        }));

    } catch (error) {
        console.error("Failed to get AI recommendations:", error);
        // Fallback to the default mock data, which has some pre-set recommendations.
        // In a real app, you might want to return an empty recommendation list or handle the error differently.
        return mockUsers;
    }
};


export const generateKnowledgeArticle = async (topic: string, targetAudience: string): Promise<{ title: string; category: string; summary: string; }> => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const prompt = `
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: Type.OBJECT,
                    properties: {
                        title: { type: Type.STRING, description: 'A compelling title for the article.' },
                        category: { type: Type.STRING, description: 'A relevant business category (e.g., Technology, Market Insight, Regulation, AI Generated).' },
                        summary: { type: Type.STRING, description: 'The generated article summary, around 100-150 words.' },
                    },
        ...
        throw new Error("Could not generate article. The AI service may be temporarily unavailable.");
    }
};

export const generateStoryDraft = async (topic: string, audience: string): Promise<{ title: string; summary: string; tags: string[] }> => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const prompt = `
        ...
        throw new Error("Could not generate story draft. The AI service may be temporarily unavailable.");
    }
};

export const generateDashboardInsights = async (userProfile: UserProfile): Promise<string[]> => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const prompt = `
